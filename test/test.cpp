...
void test_roll() {
  ...
  for (int i = 0; i < testQty; ++i) {
    ...
    if ((result < statDieQty) || (result > (statDieQty * statDieSides))) {
      std::cout << "Test 1 Failed: Value out of range. Result: " << result
                << std::endl;
      return;
    }
  }
  ...
  if ((average < testConfidenceLowerRange)
      || (average > testConfidenceUpperRange)) {
    std::cout
  ...
  if (result < 1) {
    std::cout
      << "Test 3 Failed: Rolling 0d6 should get adjusted to 1d6. Result: "
      << result << std::endl;
    return;
  }
  ...
  if (result < 1) {
    std::cout
      << "Test 3 Failed: Rolling 1d0 should get adjusted to 1d2. Result: "
      << result << std::endl;
    return;
  }
  std::cout
    << "Test 3 Passed: Edge cases are handled correctly.\n"
